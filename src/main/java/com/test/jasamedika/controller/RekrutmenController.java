...
@RestController
@RequestMapping("/pegawai/admin-tambah-pegawai")
public class RekrutmenController {
    ...
    @PostMapping
    public Object save(HttpServletRequest hsrequest, Authentication auth, @Valid @RequestBody PegawaiRequest request,
            Errors errors) {
        if (hsrequest.isUserInRole("ROLE_ADMIN") || repository.findByEmail(auth.getName()).get().getJabatan()
            ...
            return service.save(request, errors);
        }
